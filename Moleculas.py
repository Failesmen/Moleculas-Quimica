elementos = {"H": 1.0079, "He": 4.0026, "Li": 6.941, "Be": 9.0122,
...
def main():
    while True:
        raw = input('Digite a molecula:')
        formula = raw + "?"
        massa = 0
        for i in range(len(formula)):
            if upper_case(formula[i]):
                if formula[i+1] == "?":
                    massa += elementos[formula[i]]
                elif upper_case(formula[i+1]):
                    massa += elementos[formula[i]]
                elif numero(formula[i+1]):
                    if formula[i+2] == "?":
                        massa += int(formula[i+1]) * elementos[formula[i]]
                    if upper_case(formula[i+2]):
                        massa += int(formula[i+1]) * elementos[formula[i]]
                    elif numero(formula[i+2]):
                        massa += int(formula[i+1] + formula[i+2]) * elementos[formula[i]]
                elif lower_case(formula[i+1]):
                    if formula[i+2] == "?":
                        massa += elementos[formula[i] + formula[i+1]]
                    if upper_case(formula[i+2]):
                        massa += elementos[formula[i] + formula[i+1]]
                    elif numero(formula[i+2]):
                        if formula[i+3] == "?":
                            massa += (int(formula[i+2]) *
                                     elementos[formula[i] + formula[i+1]])
                        if upper_case(formula[i+3]):
                            massa += (int(formula[i+2]) *
                                     elementos[formula[i] + formula[i+1]])
                        elif numero(formula[i+3]):
                            massa += (int(formula[i+2] + formula[i+3]) *
                                     elementos[formula[i] + formula[i+1]])
        print("Massa molecular de", raw, "=", ("%.3f" % massa))
